...
function SeccionEmpresas() {
  ...
  async function handleCrear() {
    ...
    try {
      // Convertimos strings vacíos en null para que el backend no se queje
    ...
    <>
      ...
      <Card title="Registrar contacto">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
          <Field label="ID Profesor">
            <input type="number" value={form.profesor_id} onChange={e => setForm(p => ({ ...p, profesor_id: e.target.value }))} placeholder="1" style={inputStyle} />
          </Field>
          <Field label="ID Empresa">
            <input type="number" value={form.empresa_id} onChange={e => setForm(p => ({ ...p, empresa_id: e.target.value }))} placeholder="1" style={inputStyle} />
          </Field>
          ...
          : (
            <table style={tableStyle}>
              ...
              <tbody>
                {contactos.map(c => (
                  <tr key={c.id}>
                    <td style={tdStyle}>{c.profesor_id}</td>
                    <td style={tdStyle}>{c.empresa_id}</td>
    ...
    <>
      ...
      <Card title="Nueva plaza">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '10px', alignItems: 'flex-end' }}>
          ...
          <Field label="ID Ciclo">
            <input type="number" value={form.ciclo_id} onChange={e => setForm(p => ({ ...p, ciclo_id: e.target.value }))} placeholder="1" style={inputStyle} />
          </Field>
                ...
                {plazas.map(p => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td style={tdStyle}>{p.empresa_id}</td>
...
function SeccionAsignaciones() {
  const [form, setForm] = useState({ alumno_id: '', plaza_id: '', tutor_laboral_id: '' });
  const [alertMsg, alertType, showAlert] = useAlert();
 
  async function handleAsignar() {
    const { alumno_id, plaza_id, tutor_laboral_id } = form;
    if (!alumno_id || !plaza_id) { showAlert('Alumno y plaza son obligatorios', 'error'); return; }
    try {
      await createAsignacion(
        parseInt(alumno_id),
        parseInt(plaza_id),
        tutor_laboral_id ? parseInt(tutor_laboral_id) : null
      );
      showAlert('Alumno asignado correctamente', 'success');
      setForm({ alumno_id: '', plaza_id: '', tutor_laboral_id: '' });
    } catch (err) {
    ...
    <>
      ...
      <Card title="Asignar alumno a plaza">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '10px', alignItems: 'flex-end' }}>
          ...
          <Field label="ID Plaza">
            <input type="number" value={form.plaza_id} onChange={e => setForm(p => ({ ...p, plaza_id: e.target.value }))} placeholder="1" style={inputStyle} />
          </Field>
          <Field label="ID Tutor laboral (opcional)">
            <input type="number" value={form.tutor_laboral_id} onChange={e => setForm(p => ({ ...p, tutor_laboral_id: e.target.value }))} placeholder="—" style={inputStyle} />
          </Field>
      ...
      <Card>
        <p style={{ fontSize: '13px', color: '#888' }}>
          💡 Para ver los IDs de alumnos y plazas disponibles, consulta las secciones
          <strong> Plazas</strong> y pregunta a tu compañero los IDs de alumnos importados.
        </p>
      </Card>
    </>
  );
